...
@Abbrev("KENDO_NESTED")
public class KendoNestedTool extends Tool {
	...
	public KendoNestedTool(String name, Tool next, CommandLine commandLine) {
	...
	public void acquire(AcquireEvent ae) {
		//Util.printf("got turn for " +ae.getThread().getTid()+"with clock "+getLogicalClock(ae.getThread()));
		ShadowThread thread = ae.getThread();
		incrementLogicalClock(thread, false);
	...
	public void release(ReleaseEvent re) {
		//Util.printf("Releasing lock by thread "+ re.getThread() +" at logical time "+ getLogicalClock(re.getThread()));
		ShadowThread thread = re.getThread();
		incrementLogicalClock(thread, false);
		resumeLogicalClock(thread);
